...
usage() {
  cat <<'EOF'
Build a Linux SpiderVenoms release archive containing the installed managed bundle artifacts.

Usage:
  package-spidervenoms-linux-release.sh [--version <version>] [--out-dir <dir>]

Outputs:
  <out-dir>/spidervenoms-managed-local-linux-<arch>.tar.gz
  <out-dir>/spidervenoms-managed-local-linux-<arch>.tar.gz.sha256
EOF
}
...
ARCHIVE_BASENAME="spidervenoms-managed-local-linux-${ARCH_LABEL}"
